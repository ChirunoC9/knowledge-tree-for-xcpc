...
public:
    ...
    Scanner(std::FILE *f = stdin, std::size_t enough_buffer_size = 1 << 25)
        : __file(f), __buffer(new char[enough_buffer_size + 32]), __buffer_head(__buffer),
        __buffer_tail(__buffer + std::fread(__buffer, sizeof(char), enough_buffer_size + 32, __file)) {
        *__buffer_tail = '\0';
    }
    ~Scanner() {
        delete[] __buffer;
    ...
    std::enable_if_t<
            std::is_integral_v<__ty> 
        && !std::is_same_v<__ty, char>
    ...
    template <typename... __type_list>
    bool read(__type_list &...__n_list) {
